...
sequenceDiagram
    ...
    Job->>Action: 記事生成依頼
    Action->>ClaudeAPI: 記事生成リクエスト
    ClaudeAPI-->>Action: 生成記事

    loop 最大3回（初回+リトライ2回）
        Action->>GeminiAPI: レビューリクエスト
        GeminiAPI-->>Action: レビュー結果

        alt レビュー合格
            Action->>DB: 記事とレビュー結果を保存
            Action-->>Job: 完了
        else レビュー不合格 & リトライ可能
            Action->>ClaudeAPI: 修正指示付き再生成
            ClaudeAPI-->>Action: 修正記事
        else レビュー不合格 & リトライ上限
...
**責務**:
- Pull Request情報から記事を生成する（Claude Sonnet 4.5使用）
- 生成された記事を品質レビューする（Gemini 2.5 Pro使用）
- レビュー不合格時に記事を再生成する（Claude Sonnet 4.5使用）
- レビュー結果を構造化データとして返却する

**使用するAIモデル**:
- **記事生成**: Claude Sonnet 4.5（Anthropic API）
- **品質レビュー**: Gemini 2.5 Pro（Google Gemini API）
...
**API統合の考慮事項**:
- Anthropic APIとGemini APIの認証情報を個別に管理
- 各APIのレスポンス形式の違いを吸収する
- エラーハンドリングをAPI毎に実装
...
レビューAIは以下の手順で記事を評価する:

1. **ガイドライン準拠の確認**:
   - 記事構成（Title、Context、Technical Detail）の有無を確認
   - カスタムMarkdown構文の正確性を検証:
     - コードブロック前後の空行（重要）
     - ファイル名付きシンタックスハイライト
     - GitHubリンク記法（コミットID、PR、Issue）
   - 対象読者への適合性を評価

2. **技術的整合性の検証**:
   - ステップ1: 記事内のコードブロックとDiff内容を照合
   - ステップ2: 技術用語がPR情報と一致するか確認
   - ステップ3: 技術的主張がPR情報とDiffで裏付けられるか検証

3. **ハルシネーション検出**:
   - ステップ1: 記事内の主張がPR情報（Title、Description、Diff）で裏付けられるか確認
   - ステップ2: 数値・固有名詞（PR番号、コミットID、Issue番号等）の正確性を検証
   - ステップ3: 記事のタイトル・説明がPR内容と一致するか確認

**判定基準の明確化**:

各観点について、以下の明確な判定基準を設定:

| 観点 | PASS | WARNING | FAIL |
|------|------|---------|------|
| 記事構成 | 3要素すべて明確 | 3要素はあるが説明不十分 | いずれかの要素が欠落 |
| カスタムMarkdown | すべての構文が正しい | GitHubリンク記法が不完全 | コードブロック前後の空行なし |
| Diff照合 | Diff内容を正確に反映 | Diffの一部省略だが理解可能 | Diffに存在しないコードを引用 |
| 技術用語 | 正確で適切な用語 | 用語選択が最適でないが理解可能 | 明らかな技術用語の誤用 |
| 事実の裏付け | すべての主張がPR情報で裏付け | 妥当な推測だがPR情報に明示なし | PR情報にない重要な主張 |

**評価例の提供**:

プロンプトには、各観点のPASS/WARNING/FAIL例を具体的に記載:

- ガイドライン準拠: コードブロック空行の有無、記事構成の例
- 技術的整合性: Diffとコード引用の一致/不一致の例
- ハルシネーション: 裏付けのある主張/推測/根拠のない主張の例
...
**Gemini API固有のエラーケース**:
- **認証エラー**: API キーが無効または期限切れ
- **レート制限**: リクエスト数が上限を超過
- **レスポンス形式エラー**: 期待したJSON構造と異なる
- **タイムアウト**: Gemini APIのレスポンスが遅延
...
- **API制限**:
  - Anthropic API（記事生成）の利用制限に依存する
  - Google Gemini API（レビュー）の利用制限に依存する
  - 両APIのレート制限やクォータを考慮する必要がある
- **複数APIの統合**:
  - 異なるプロバイダーのAPI認証・接続管理が必要
  - APIレスポンス形式の違いを吸収する処理が必要
  - 各APIのエラーハンドリング方法が異なる可能性がある
...
- **REVIEWER_MODEL_NAME**: レビューモデル名（"Gemini 3 Pro"）
...
記事生成には Claude Sonnet 4.5 を使用し、品質レビューには Gemini 2.5 Pro を使用することで、以下のメリットを得る:
...
- **高度な推論能力**: Gemini 2.5 Proは複雑なコードや技術的内容の理解に優れており、技術記事のレビューに適している

### 12.4 API統合の実装上の考慮事項

**認証情報の管理**:
- Anthropic APIキーとGemini APIキーを環境変数で個別に管理
- 各APIキーの有効性を起動時にチェック

**レスポンス形式の統一**:
- Gemini APIのレスポンスを内部の統一形式に変換
- JSON構造化出力の要求方法がAPI毎に異なる可能性を考慮

**エラーハンドリングの統一**:
- 各APIのエラーコードを内部エラータイプにマッピング
- リトライ可能なエラーと致命的なエラーを区別
...
| 1.2 | 2026-01-07 | Gemini API統合に関する技術的制約、エラーハンドリング、API統合の考慮事項を追加 | - |
| 1.3 | 2026-01-07 | レビューモデルをGemini 2.5 Proに変更（Gemini 3 ProはAPI未対応のため） | - |
| 1.4 | 2026-01-07 | レビュープロンプトを改善：評価基準の具体化、ハルシネーション検出手順の明確化、評価例の追加、カスタムMarkdown構文評価の強化 | - |
